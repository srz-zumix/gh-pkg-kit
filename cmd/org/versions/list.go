...
// NewListCmd creates a command to list package versions for an organization
func NewListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "List package versions for an organization",
		Long:  `Lists package versions for a package owned by an organization.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			versions, err := gh.ListOrgPackageVersions(ctx, g, repo, packageType, packageName, state)
			if err != nil {
				return err
			}
